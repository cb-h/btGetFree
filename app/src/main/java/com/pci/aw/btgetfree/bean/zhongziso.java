...
public class zhongziso implements Runnable {

    private String url = "https://www.zhongzilou.com/list/";
    private ArrayList list ;
    private String tag;
    private int page;

    public zhongziso(String tag, ArrayList list, int page) {
        this.list = list;
        this.tag = tag;
        this.page = page;
    }
    ...
    public void run() {
        try {
            MyLog.e("zhongziso running");
            Document document= Jsoup.connect( url + tag + "/"+ page)
                    .timeout(10000).userAgent("UA").validateTLSCertificates(false).get();//10s的请求超时
            MyLog.e("https bturl  document.html",document.html());
            Elements ul = document.select("table.table");

            MyLog.e("   es   "+ul.first().html());
            for (Element e:ul){
                MyLog.e(e.text());
